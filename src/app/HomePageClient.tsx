...
// Interactive homepage implementation; route metadata lives in page.tsx.
...
export default function HomeNewPage() {
    ...
    useEffect(() => {
        ...
        async function load() {
            try {
                const [productsRes, artistsRes, shoutsRes, polaroidsRes, artistFeaturesRes, journalRes] = await Promise.all([
                ...
                const [productsJson, artistsJson, shoutsJson, polaroidsJson, artistFeaturesJson, journalJson] = await Promise.all([
                    productsRes.json(),
                    artistsRes.json(),
                    shoutsRes.json(),
                    polaroidsRes.json(),
                ...
                setJournalPosts(Array.isArray(journalJson.journal) ? journalJson.journal.slice(0, 3) : []);
            } catch {
                if (!mounted) {
                    return;
                }
    ...
    return (
        <main className="min-h-screen bg-[#080808] text-white">
            ...
            <SceneTicker />
            <QuickRacks products={products} loading={loading} />
            ...
            <CommunityScene artists={artistList} posts={journalPosts} loading={loading} />
    ...
    return (
        <section className="relative overflow-hidden border-b border-neutral-800 bg-[#080808]">
            <div className="relative min-h-[430px] overflow-hidden md:min-h-[455px] xl:min-h-[475px]">
                ...
                <div className="relative z-10 flex min-h-[430px] items-center px-5 py-8 md:min-h-[455px] md:px-10 xl:min-h-[475px] xl:px-12">
                    <div className="max-w-[680px]">
                        <h1 className="max-w-[680px] font-black leading-[0.9] text-[#f2f0ea] drop-shadow-[0_4px_0_rgba(0,0,0,0.45)] [font-stretch:condensed] [text-shadow:0_8px_20px_rgba(0,0,0,0.62)]">
                            <span
                                className="block text-[58px] text-[#f2f0ea] md:whitespace-nowrap md:text-[68px] xl:text-[76px]"
                            >
                                Build the drop.
                            </span>
                            <span
                                className="block text-[56px] text-[#ef0000] md:whitespace-nowrap md:text-[66px] xl:text-[74px]"
                            >
                                Back the band.
                            </span>
                        </h1>
    ...
    return (
        <div className="relative overflow-hidden bg-[#101010] p-6 text-white md:-mt-3 md:rounded-t-md">
            ...
            <div className="relative">
                ...
                <Link href="/start" className="mx-auto mt-4 flex h-10 max-w-[210px] items-center justify-center gap-2 rounded bg-lime-300 text-sm font-black text-black shadow-[0_0_28px_rgba(190,242,100,0.22)] hover:bg-lime-200">
...
function FeatureStack() {
    const features = [
        {
            title: "Live drops",
            body: "Published products pulled directly from the live catalogue.",
...
function QuickRacks({ products, loading }: { products: Product[]; loading: boolean }) {
    const [selectedCategory, setSelectedCategory] = useState("tees");
    const lanes = [
        { id: "tees", title: "Tees", href: "/category/tees", note: "front-row staples", icon: Shirt },
        { id: "hoodies", title: "Hoodies", href: "/category/hoodies", note: "cold-night merch", icon: Package },
        { id: "tanks", title: "Tank Tops", href: "/category/tanks", note: "pit-ready layers", icon: BicepsFlexed },
        { id: "bags", title: "Bags", href: "/category/bags", note: "carry the scene", icon: ShoppingBag },
        { id: "posters", title: "Posters", href: "/category/posters", note: "flyer wall finds", icon: Camera },
        { id: "hats", title: "Hats", href: "/category/hats", note: "top-shelf headwear", icon: HatGlasses },
    ];
    const activeLane = lanes.find((lane) => lane.id === selectedCategory) ?? lanes[0];
    const cards = loading
        ? Array.from({ length: 6 })
        : products
            .filter((product) => product.category?.trim().toLowerCase() === selectedCategory)
            .slice(0, 12);
        ...
        <section className="border-b border-neutral-200 bg-[#f2f0ea] px-4 py-12 text-black md:px-8 md:py-16">
            <div className="mx-auto max-w-[1600px]">
                <div className="flex flex-wrap items-end justify-between gap-5">
                    ...
                    <Link href={activeLane.href} className="inline-flex items-center gap-2 border border-black px-5 py-3 text-sm font-black uppercase tracking-[0.12em] transition hover:bg-lime-300">
                        View all {activeLane.title} <ArrowRight className="h-4 w-4" />
                    </Link>
                </div>
                <div className="mt-8 grid grid-cols-2 gap-3 lg:grid-cols-6" role="group" aria-label="Filter products by category">
                    {lanes.map((lane) => {
                        const Icon = lane.icon;
                        const selected = lane.id === selectedCategory;
                        return (
                            <button
                                key={lane.id}
                                type="button"
                                aria-pressed={selected}
                                onClick={() => setSelectedCategory(lane.id)}
                                className={`min-h-[126px] border p-5 text-left shadow-[6px_6px_0_rgba(0,0,0,0.08)] transition hover:-translate-y-0.5 hover:border-lime-400 ${selected ? "border-black bg-lime-300" : "border-black/15 bg-white"}`}
                            >
    ...
    return (
        <section className="overflow-hidden border-y border-black bg-[#ef0000] text-black">
            <div
                ...
                style={{ animation: "home-new-marquee 34s linear infinite" }}
            >
...
function RotatingArtistFeature({
    ...
    const fallbackFeatures = fallbackArtists.map((artist, index) => ({
        ...
        photos: artist.image ? [{ id: `artist-${index}`, image: artist.image, caption: artist.display_name ?? artist.name }] : [],
    ...
    const activePhotos = active?.photos?.length ? active.photos : [];
            ...
            <div className="bg-neutral-950">
                ...
                <div className="grid grid-cols-[1fr_150px] gap-4 p-6 md:p-12">
                    ...
                    <div className="border border-neutral-800 bg-black p-4">
                        ...
                        <p className="mt-4 line-clamp-4 text-sm font-black">{bundleProducts[0]?.title ?? "No bundle products live yet"}</p>
                        <p className="mt-3 text-4xl font-black text-lime-300">{typeof bundleProducts[0]?.price === "number" ? `$${bundleProducts[0].price}` : "—"}</p>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section className="border-b border-neutral-800 bg-black">
            ...
            <div className="grid border-t border-neutral-800 md:grid-cols-4">
                {loading ? Array.from({ length: 4 }).map((_, index) => (
                    <div key={index} className="min-h-[360px] animate-pulse border-b border-r border-neutral-800 bg-neutral-950" />
                )) : null}
                ...
                {!loading && posts.length === 0 ? (
                    <div className="min-h-[260px] border-r border-neutral-800 p-6 md:col-span-4 md:p-10">
                        <p className="text-3xl font-black uppercase">The backstage wall is ready.</p>
                        <p className="mt-3 max-w-xl text-sm leading-6 text-neutral-400">Photos from shows, community events and life behind Merch Tent will appear here as we publish them.</p>
                    </div>
                ) : null}
            </div>
        </section>
...
function RealLifeAndReviews({ fanShouts }: { fanShouts: FanShout[] }) {
    const shouts = fanShouts;
    const ratings = shouts.flatMap((shout) => typeof shout.rating === "number" ? [shout.rating] : []);
    ...
    return (
        <section className="grid border-b border-neutral-800 bg-black lg:grid-cols-[320px_1fr]">
            <div className="border-b border-neutral-800 p-6 md:p-12 lg:border-b-0 lg:border-r">
                ...
                <p className="mt-8 text-7xl font-black">{average ? average.toFixed(1) : "—"}</p>
                {average ? <div className="mt-2 flex text-lime-300">
                    {Array.from({ length: 5 }).map((_, index) => <Star key={index} className={`h-5 w-5 ${index < Math.round(average) ? "fill-current" : "opacity-25"}`} />)}
                </div> : null}
                <p className="mt-3 text-sm text-neutral-400">{ratings.length ? `${ratings.length} verified fan rating${ratings.length === 1 ? "" : "s"}.` : "No verified ratings yet."}</p>
            </div>
            <div className="grid md:grid-cols-3">
                ...
                {shouts.length === 0 ? (
                    <div className="min-h-[300px] border-b border-r border-neutral-800 bg-neutral-950 p-6 md:col-span-3 md:p-10">
                        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-lime-300">[ fan reviews ]</p>
                        <p className="mt-8 max-w-2xl text-3xl font-black leading-tight">The first verified reviews will appear here after customers receive their orders.</p>
                    </div>
                ) : null}
            </div>
        </section>
...
function CommunityScene({ artists, posts, loading }: { artists: Artist[]; posts: JournalPost[]; loading: boolean }) {
        ...
        <section className="relative grid border-b border-neutral-800 bg-[#101010] lg:grid-cols-[1.55fr_0.8fr]">
            ...
            <div className="relative border-b border-neutral-800 p-6 md:p-10 lg:border-b-0 lg:border-r">
                <div className="flex items-end justify-between gap-4">
                    ...
                    <Link href="/journal" className="inline-flex items-center gap-2 text-sm font-black text-lime-300">
                        View all posts <ArrowRight className="h-4 w-4" />
                    </Link>
                </div>
                <div className="mt-5 grid gap-4 md:grid-cols-3">
                    {posts.map((post) => (
                        <Link key={post.id} href={`/journal/${post.slug}`} className="group overflow-hidden border border-white/10 bg-black/35 hover:border-lime-300">
                            <div className="relative aspect-[4/3]">
                                <JournalCoverImage post={post} />
                                <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-transparent to-transparent" />
                                <span className="absolute left-3 top-3 bg-black/75 px-2 py-1 text-[10px] font-bold uppercase tracking-[0.12em] text-lime-300">{post.artist ?? "Merch Tent journal"}</span>
                            </div>
                            <div className="p-4">
                                <h3 className="line-clamp-2 text-lg font-bold leading-6 text-white">{post.title}</h3>
                                <p className="mt-2 line-clamp-2 text-sm leading-6 text-neutral-400">{post.description || "Read the latest story from the scene."}</p>
                            </div>
                        </Link>
                    ))}
            ...
            <aside className="relative flex items-center p-6 md:p-10">
                <div>
                    <p className="text-[11px] font-black uppercase tracking-[0.28em] text-lime-300">Scene journal</p>
                    <p className="mt-5 max-w-sm text-2xl font-black leading-tight text-neutral-100">Artist stories, launch notes and voices from the Australian live scene.</p>
                    <div className="mt-6 flex items-center gap-3">
                        ...
                        <p className="text-sm text-neutral-400">{artists.length ? `${artists.length} featured artist${artists.length === 1 ? "" : "s"}` : "Artists joining soon"}</p>
                    </div>
                </div>
            </aside>
        </section>
...
function JournalCoverImage({ post }: { post: JournalPost }) {
    const placeholder = "/merch-placeholder.svg";
    const fallback = post.avatar || placeholder;
    const [src, setSrc] = useState(post.image || fallback);
    ...
    return (
        <Image
            ...
            fill
            sizes="28vw"
            className="object-cover opacity-78 transition duration-300 group-hover:scale-105 group-hover:opacity-100"
            onError={() => setSrc((current) => current === fallback ? placeholder : fallback)}
        />
    );
}
...
function JoinTheListNew() {
    ...
    async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            trackMarketingEvent("newsletter_signup", { source: "home-new:join-the-list" });
            setEmail("");
            setName("");
        } catch {
    ...
    return (
        ...
        >
            <div className={`relative overflow-hidden rounded-t-[6px] bg-[#d8d5cc] ${compact ? "aspect-[4/4.45]" : "aspect-[4/5]"}`}>
                <span className="absolute left-2 top-2 z-10 rounded-sm bg-lime-300 px-2 py-1 text-[10px] font-black leading-none text-black">LIVE DROP</span>
                {product?.image ? (
