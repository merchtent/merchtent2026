...
export const COUNTRY_CODES = new Set<string>(COUNTRY_OPTIONS.map(([code]) => code));
...
export const AUSTRALIAN_STATE_CODES = new Set<string>(AUSTRALIAN_STATE_OPTIONS.map(([code]) => code));

export function normaliseCountryCode(value: unknown, fallback = "AU") {
    const candidate = String(value ?? "").trim().toUpperCase();
    return COUNTRY_CODES.has(candidate) ? candidate : fallback;
}

export function normaliseAustralianState(value: unknown) {
    const candidate = String(value ?? "").trim().toUpperCase();
    return AUSTRALIAN_STATE_OPTIONS.find(
        ([code, name]) => code === candidate || name.toUpperCase() === candidate
    )?.[0] ?? "";
}
