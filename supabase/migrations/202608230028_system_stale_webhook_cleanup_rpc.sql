create or replace function public.system_mark_stale_stripe_webhooks_failed(
...
begin
    ...
    updated_events as (
        update public.stripe_webhook_events swe
           set status = 'failed',
               failed_at = now(),
               last_error = 'Marked failed by scheduled maintenance after stale processing timeout.'
    ...
    if v_marked_count > 0 then
        perform public.log_platform_event(
            'stripe',
            'scheduled_stale_stripe_webhooks_marked_failed',
            'warning',
            null,
            null,
            null,
            ...
            null,
            null,
            'Scheduled maintenance marked stale Stripe webhook processing events as failed.',
            jsonb_build_object(
                'stale_after_minutes',
...
revoke all on function public.system_mark_stale_stripe_webhooks_failed(integer, integer) from public;
revoke all on function public.system_mark_stale_stripe_webhooks_failed(integer, integer) from anon;
grant execute on function public.system_mark_stale_stripe_webhooks_failed(integer, integer) to service_role;
