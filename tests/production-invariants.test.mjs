...
test("Merch credit redemption is ledger-backed, locked, and idempotent", () => {
  ...
  const merchCreditConstants = read("src/lib/merch-credits/constants.ts");
  ...
  assert.match(merchCreditConstants, /MERCH_CREDIT_REDEMPTION_POINTS = 20/);
  assert.match(merchCreditConstants, /MERCH_CREDIT_REDEMPTION_CENTS = 2_000/);
  assert.match(merchCreditConstants, /MERCH_CREDITS_EARNED_PER_ITEM = 3/);
  ...
  assert.match(checkout, /calculateMerchCreditDiscountCents/);
  ...
  assert.match(checkoutForm, /Each credit is worth \$1/);
  assert.match(checkoutForm, /Shipping is excluded/);
  ...
  assert.match(dashboard, /20 credits gives you \$20 off merchandise at checkout/);
...
test("Checkout validates fulfillment-grade customer and shipping fields", () => {
  ...
  assert.match(checkoutForm, /COUNTRY_OPTIONS\.map/);
  assert.match(checkoutForm, /fd\.set\("checkout_attempt_id", crypto\.randomUUID\(\)\)/);
  assert.match(checkoutForm, /aria-label="Country"/);
  assert.match(checkoutForm, /AUSTRALIAN_STATE_OPTIONS\.map/);
  ...
  assert.match(checkoutForm, /const savedCountry = normaliseCountryCode\(savedDraft\.country, emptyDraft\.country\)/);
  assert.match(checkoutForm, /country: savedCountry/);
...
test("Product edits validate readiness before publishing", () => {
  ...
  assert.doesNotMatch(productsPage, /lifecycle\?\.readiness_notes/);
...
test("Designer saves include validation status and generation audit events", () => {
  ...
  assert.match(designerAction, /readiness_notes: null/);
